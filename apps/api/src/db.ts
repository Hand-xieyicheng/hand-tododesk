...
type SchemaExecutor = (sql: string) => Promise<unknown>;

export async function ensureAiSchema(executor: SchemaExecutor = execute) {
  await executor(
...
async function ensureIncrementalSchema() {
  ...
  await ensureColumn("UserThemePreference", "floatingCardThemeId", "ALTER TABLE `UserThemePreference` ADD COLUMN `floatingCardThemeId` VARCHAR(191) NOT NULL DEFAULT 'warm-paper'");
  await ensureColumn("UserThemePreference", "floatingCardViewMode", "ALTER TABLE `UserThemePreference` ADD COLUMN `floatingCardViewMode` VARCHAR(191) NOT NULL DEFAULT 'list'");
